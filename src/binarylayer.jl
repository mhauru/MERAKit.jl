...
inputspace(layer::BinaryLayer) = space(layer.disentangler, 1)
outputspace(layer::BinaryLayer) = space(layer.isometry, 3)'
...
function minimize_expectation_isometry(h::SquareTensorMap{3}, layer::BinaryLayer, rho)
    ...
    @tensor w[-1,-2,-3] := conj(U[-1,-2,1]) * conj(Vt[1,-3])
