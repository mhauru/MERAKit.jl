...
Base.:+(x::ModifiedBinaryOp, y::ModifiedBinaryOp) = ModifiedBinaryOp((xi + yi for (xi, yi) in zip(x, y))...)
...
# The thermal density matrix needs to wrapped in a ModifiedBinaryOp
"""
...
"""
function thermal_densitymatrix(m::ModifiedBinaryMERA, depth)
    # Call the method of the supertype GenericMERA.
    rho = invoke(thermal_densitymatrix, Tuple{GenericMERA, typeof(depth)}, m, depth)
...
"""
function ascend(op::ModifiedBinaryOp, layer::ModifiedBinaryLayer, pos=:avg)
    u, wl, wr = layer
    op_mid, op_gap = op
    if in(pos, (:left, :l, :L))
        # Cost: 2X^7 + 3X^6 + 1X^5
...
"""
function minimize_expectation_layer(h, layer::ModifiedBinaryLayer, rho, pars; do_disentanglers=true)
    for i in 1:pars[:layer_iters]
        ...
        for j in 1:pars[:isometry_iters]
            layer = minimize_expectation_isometry_left(h, layer, rho)
            layer = minimize_expectation_isometry_right(h, layer, rho)
        end
    end
    return layer
end
...
"""
function minimize_expectation_disentangler(h, layer::ModifiedBinaryLayer, rho)
    wl = layer.isometry_left
    wr = layer.isometry_right
...
minimize the expectation of a threesite operator `h`.
"""
function minimize_expectation_isometry_left(h, layer::ModifiedBinaryLayer, rho)
...
minimize the expectation of a threesite operator `h`.
"""
function minimize_expectation_isometry_right(h, layer::ModifiedBinaryLayer, rho)
    u = layer.disentangler
    wl = layer.isometry_left
